...
from d51.django.apps.sharing.models import URL, Share
...
class TestOfURL(TestCase):
    ...
    def test_of_share_to_service(self):
        ...
        services.create_share('gary_service', user, from_url=url)
        self.mox.ReplayAll()
    ...
    def test_of_send(self):
        ...
        mock_share.send().AndReturn(random_number)
        url = URL()
        user = User.objects.create(
            username='gary%d'%random.randint(1,1000),
        ) 
        self.mox.StubOutWithMock(services, 'create_share')
        services.create_share('gary_service', user, from_url=url).AndReturn(mock_share)
        self.mox.ReplayAll()
